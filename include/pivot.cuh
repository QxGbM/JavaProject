...
__device__ unsigned int blockAllFindRowPivot (const unsigned int row, const matrixEntriesT *matrix, const unsigned int nx, const unsigned int ld, const unsigned int ny)
{
  /*
  ...
  * The total amount of allocated shared memory is tile_size * (size per matrix entry + size of unsigned int).
  ...
  */
  ...
  {
    const unsigned int index = row + i * tile_size + lane_id;
  ...
  for (unsigned int i = 0; i < n; i++) 
  {
    ...
    {
      const unsigned int index = shm_index[slot];
...
template <class matrixEntriesT>
__device__ void blockApplyPivot (thread_group g, const unsigned int *pivot, const bool recover, matrixEntriesT *matrix, 
  const unsigned int nx, const unsigned int ld, const unsigned int ny)
{
